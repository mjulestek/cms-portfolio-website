...
async function missingIds(model: 'tag' | 'skill' | 'videoAsset' | 'blogCategory', ids: string[]) {
  ...
  const rows =
    model === 'tag'
      ...
      : model === 'skill'
        ...
        : model === 'blogCategory'
          ? await prisma.blogCategory.findMany({ where: { id: { in: uniqueIds } }, select: { id: true } })
...
async function ensureIdsExist(model: 'tag' | 'skill' | 'videoAsset' | 'blogCategory', field: string, ids?: string[]): Promise<ReferenceCheck> {
...
export async function validateBlogReferences(input: {
  ...
  for (const check of [
    await ensureIdsExist('tag', 'tagIds', input.tagIds),
    await ensureIdsExist('blogCategory', 'categoryId', input.categoryId ? [input.categoryId] : []),
    await ensureIdsExist('videoAsset', 'videoIds', input.videoIds),
    await ensureMediaKeyExists('coverImageKey', input.coverImageKey, ['IMAGE', 'ICON', 'AVATAR']),
  ]) {
    if (!check.ok) return check;
...
export async function validateHomepageReferences(input?: { featuredVideoId?: string | null; heroImageKey?: string | null } | string | null): Promise<ReferenceCheck> {
  const featuredVideoId = typeof input === 'string' ? input : input?.featuredVideoId;
  const heroImageKey = typeof input === 'string' ? null : input?.heroImageKey;

  for (const check of [
    await ensureIdsExist('videoAsset', 'featuredVideoId', featuredVideoId ? [featuredVideoId] : []),
    await ensureMediaKeyExists('heroImageKey', heroImageKey, ['IMAGE', 'ICON', 'AVATAR']),
  ]) {
    if (!check.ok) return check;
  }
  return { ok: true };
